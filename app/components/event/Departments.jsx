...
const handleDeleteBlog = async (id) => {
    ...
    try {
      const response = await axios.get(`https://skytextiles.in/api/v1/event/delete/${id}`);
                      ...
                      <TableBody>
                        {loading ? (
                          ...
                          departments.map((department, index) => (
                           ...
                           <TableCell>
  {(() => {
    const validDomains = ["res.cloudinary.com", "skytextiles.in", "www.skytextiles.in"];
    const isValidSrc = department.image
      ? validDomains.some(domain => department.image.includes(domain))
      : false;
    return (
      <Image
        src={isValidSrc ? department.image : "/placeholder.jpg"}
